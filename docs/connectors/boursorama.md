...
Le connecteur BoursoBank permet la synchronisation automatique des transactions et soldes depuis l'interface web BoursoBank en utilisant Playwright pour automatiser un navigateur.
...
**Documentation de recherche** : Voir `docs/connectors/boursorama-research.md` pour l'évaluation complète des options.

## Installation

Le connecteur nécessite Playwright :
...
```python
{
    "username": "identifiant_boursorama",  # Identifiant BoursoBank
...
2. Naviguer vers la page de connexion BoursoBank
...
Les sélecteurs CSS utilisés pour scraper les données peuvent changer si BoursoBank modifie son interface. Le connecteur utilise plusieurs sélecteurs alternatifs pour améliorer la robustesse, mais peut nécessiter des ajustements si l'interface change significativement.
...
```python
from finance.connectors import BoursoBankConnector
...
bank_connection = BankConnection.objects.get(provider="boursorama")
...
Les tests unitaires sont disponibles dans `backend/finance/connectors/tests/test_boursorama.py`.

**Couverture** :
- Authentification (succès, 2FA, erreurs, timeout)
- Synchronisation des transactions (filtrage, formatage)
...
Si l'interface BoursoBank change, les sélecteurs CSS doivent être ajustés dans :
...
- Document de recherche : `docs/connectors/boursorama-research.md`
