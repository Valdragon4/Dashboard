"""
Tests unitaires pour le connecteur Hello Bank.
...
from finance.connectors.hellobank import HelloBankConnector, PLAYWRIGHT_AVAILABLE
...
class TestHelloBankConnector(TestCase):
    """Tests pour le connecteur Hello Bank."""
    ...
    def setUp(self):
        ...
        self.connector = HelloBankConnector()
        self.credentials = {
            "username": "test_user",
    ...
    def test_provider_name(self):
        """Test que provider_name retourne 'Hello Bank'."""
        self.assertEqual(self.connector.provider_name, "Hello Bank")
    ...
    @patch('finance.connectors.hellobank.sync_playwright')
    def test_authenticate_success(self, mock_playwright):
        ...
        mock_page.url = "https://www.hellobank.fr/compte"
    ...
    def test_authenticate_requires_2fa(self, mock_playwright):
        ...
        mock_page.url = "https://www.hellobank.fr/connexion"
    ...
    def test_authenticate_timeout(self, mock_playwright):
        ...
        from finance.connectors.hellobank import PlaywrightTimeoutError
    ...
    def test_sync_transactions_success(self, mock_playwright):
        ...
        mock_cells = [
            ...
            MagicMock(inner_text=lambda: "Virement"),
            MagicMock(inner_text=lambda: "Virement depuis compte"),
    ...
    def test_sync_transactions_filter_by_date(self, mock_playwright):
        ...
        mock_cells1 = [
            MagicMock(inner_text=lambda: "01/01/2025"),
            MagicMock(inner_text=lambda: "VIREMENT"),
            MagicMock(inner_text=lambda: "Virement 1"),
            MagicMock(inner_text=lambda: "Détails virement 1"),
            MagicMock(inner_text=lambda: "100,00 €"),
        ]
        mock_cells2 = [
            MagicMock(inner_text=lambda: "15/01/2025"),
            MagicMock(inner_text=lambda: "DEBIT"),
            MagicMock(inner_text=lambda: "Débit 2"),
            MagicMock(inner_text=lambda: "Détails débit 2"),
    ...
    def test_parse_transaction_row_hello_bank_format(self):
        """Test parsing d'une ligne de transaction au format Hello Bank."""
        # Mock d'une ligne de transaction Hello Bank
        mock_row = MagicMock()
        mock_cells = [
            MagicMock(inner_text=lambda: "01/01/2025"),
            MagicMock(inner_text=lambda: "VIREMENT"),
        ...
        transaction = self.connector._parse_transaction_row(mock_row)
        
        self.assertIsNotNone(transaction)
        self.assertEqual(transaction["posted_at"].date(), datetime(2025, 1, 1).date())
        self.assertEqual(transaction["amount"], Decimal("100.00"))
        self.assertIn("Virement", transaction["description"])
        self.assertIn("raw", transaction)
        self.assertEqual(transaction["raw"]["operation_type"], "VIREMENT")
        self.assertEqual(transaction["raw"]["label_short"], "Virement")
        self.assertEqual(transaction["raw"]["label_detailed"], "Virement depuis compte")
